...
namespace NetSdrClientAppTests
{
    public class NetSdrMessageHelperTests
    {
        ...
        [Test]
        public void GetDataItemMessage_MaxLength_SetsHeaderLengthToZero()
        {
            // Arrange
            ...
            Assert.That(actualType, Is.EqualTo(type));
            Assert.That(actualLength, Is.EqualTo(0)); // Special case for DataItem
        }
        ...
        [Test]
        public void GetHeader_DataItemMaxLength_ReturnsZeroLengthHeader()
        {
            // Arrange
            var type = NetSdrMessageHelper.MsgTypes.DataItem3;
            int msgLength = 8192; // Parameters length, total length = 8194

            // Act
            var header = NetSdrMessageHelper.GetHeader(type, msgLength);
            var num = BitConverter.ToUInt16(header);
            var actualType = (NetSdrMessageHelper.MsgTypes)(num >> 13);
            var actualLength = num - ((int)actualType << 13);

            // Assert
            Assert.That(header.Length, Is.EqualTo(2));
            Assert.That(actualType, Is.EqualTo(type));
            Assert.That(actualLength, Is.EqualTo(0));
        }

        [Test]
        public void TranslateHeader_DataItemZeroLength_ParsesAsMaxLength()
        {
            // Arrange
            var type = NetSdrMessageHelper.MsgTypes.DataItem0;
            var header = BitConverter.GetBytes((ushort)((int)type << 13)); // Length = 0

            // Act
            NetSdrMessageHelper.TranslateHeader(header, out var parsedType, out var parsedLength);

            // Assert
            Assert.That(parsedType, Is.EqualTo(type));
            Assert.That(parsedLength, Is.EqualTo(8194)); // _maxDataItemMessageLength
        }

        [Test]
        public void GetMessage_ExceedsMaxMessageLength_ThrowsArgumentException()
        {
            // Arrange
            var type = NetSdrMessageHelper.MsgTypes.SetControlItem;
            var code = NetSdrMessageHelper.ControlItemCodes.None;
            byte[] parameters = new byte[8192]; // Exceeds _maxMessageLength (8191)

            // Act & Assert
            Assert.Throws<ArgumentException>(() => NetSdrMessageHelper.GetMessage(type, code, parameters));
        }
    }
}
